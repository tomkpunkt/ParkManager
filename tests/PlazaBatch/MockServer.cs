...
internal static class MockServer
{
    ...
    private sealed class PlanRequest
    {
        ...
        public int PlazaLayout { get; set; }
    ...
    private static object Plan(PlanRequest request)
    {
        ...
        if (request.SiteType == 1)
        {
            ...
            var plan = PlazaPlanner.Generate(polygon, gates,
                ...
                (PlazaLayoutMode)Math.Clamp(request.PlazaLayout, 0, 3), seed,
            ...
            return new {
                ...
                paths = plan.RoutingSegments.Select(x => new {
                    ax = x.A.x, ay = x.A.y, bx = x.B.x, by = x.B.y,
                    kind = "Hidden pedestrian route", hidden = true }).ToArray(),
